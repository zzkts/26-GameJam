...
public class LongPressHandler : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IPointerExitHandler
{
    ...
    [Header("拖拽屏蔽（由 物体拖拽 写入，无需手动设置）")]
    [Tooltip("本次按下已变成拖拽时置为 true，屏蔽点击与长按触发")]
    public bool 被拖拽屏蔽;
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        // ★ 记录点击来源：让 发射动画 知道新生成的预制体该从哪里起飞
        //   （支付钱面板上的钞票、购买商品处的猪与猫都挂了本组件）
        发射动画.记录点击来源(transform);
    ...
    private IEnumerator LongPressRoutine(PointerEventData.InputButton btn)
    {
        ...
        {
            // 拖拽已经开始 → 停止连续触发
            if (被拖拽屏蔽) yield break;

            // ★ 连发时刷新点击来源，避免长按太久后发射源失效
            发射动画.记录点击来源(transform);
